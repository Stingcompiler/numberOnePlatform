...
REST_FRAMEWORK = {
    ...
    "PAGE_SIZE": 10,
    # ── Rate Limiting ────────────────────────────────────────────────────────
    # تُطبَّق فقط على النقاط الحسّاسة (تسجيل الدخول وتغيير كلمة المرور) عبر
    # ScopedRateThrottle المُعرَّف على تلك الـ views — لا throttling عام يمسّ
    # بقية الـ API حتى لا يتأثر سير العمل الطبيعي للوحة والتطبيق.
    # القيم مرتفعة عمداً لتفادي حجب دخول جماعي شرعي (شبكة مدرسة بـ NAT مشترك)
    # مع إبطاء هجمات تخمين كلمات المرور الآلية بشكل حاسم.
...
# ─────────────────────────────────────────────────────────────────────────────
# اسم مستخدم واحد يُستثنى من ربط الجهاز. مراجعو Google قد يستخدمون أكثر من
# جهاز بنفس بيانات الدخول، وربط الجهاز الواحد كان سيمنع دخولهم ⇒ رفض التطبيق
# بحجة تعذّر الوصول. يُترك فارغاً افتراضياً أي أن الاستثناء معطّل تماماً،
# ويُفعَّل بضبط المتغيّر في بيئة الخادم فقط عند الحاجة، ويُزال بعد الموافقة.
REVIEW_ACCOUNT_USERNAME = config("REVIEW_ACCOUNT_USERNAME", default="")
